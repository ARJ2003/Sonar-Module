...
import csv
import numpy as np
data_as_python_list = list(csv.reader(open("sonar.all-data.csv")))
...
np.random.shuffle(data_without_bias_column)   # Shuffle matrix rows in place

# Extract a (208, 61) input matrix:
#   - [:, 0:-1] stands for: "all rows, all columns except the last one"
#   - Convert all strings to float
X = data_without_bias_column[:, 0:-1].astype(np.float32)
...
labels = data_without_bias_column[:, -1].reshape(-1, 1)
